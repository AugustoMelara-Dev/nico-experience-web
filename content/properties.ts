...
const propertyMediaSchema = z.object({
  ...
  (media) =>
    !media.includeInPdf ||
    (media.type === "image" && Boolean(media.pdfSrc)),
  {
...
const photo = (
  ...
  pdf?: { includeInPdf: true; pdfSrc: string },
): PropertyMedia => ({
  type: "image",
  src: image(name),
  title,
  ...
  alt,
  category,
  includeInPdf: pdf?.includeInPdf ?? false,
  pdfSrc: pdf?.pdfSrc,
})

const pdfImage = (name: string) => ({
  includeInPdf: true as const,
  pdfSrc: `/pdf-assets/casa-palac/${name}.jpg`,
})
...
export const properties = z.array(propertySchema).parse([
  {
    ...
    media: [
      photo("piscina-noche-principal", "Piscina iluminada", "Una piscina iluminada que transforma la noche en un espacio agradable para relajarse y compartir.", "Piscina iluminada de Casa Palac rodeada por terrazas de madera", "piscina", pdfImage("piscina-noche-principal")),
      ...
      photo("terraza-dia", "Terraza durante el día", "Un espacio exterior de madera para conversar y disfrutar el entorno.", "Terraza de madera con mesa y vegetación alrededor", "terraza", pdfImage("terraza-dia")),
      ...
      photo("sala-comedor", "Sala y comedor", "Sala y comedor integrados para compartir con comodidad durante la estadía.", "Sala y comedor conectados con ventanales hacia la terraza", "comedor", pdfImage("sala-comedor")),
      ...
      photo("cocina-principal", "Cocina", "Cocina disponible para preparar alimentos durante la estadía.", "Cocina con gabinetes de madera y electrodomésticos visibles", "cocina", pdfImage("cocina-principal")),
      ...
      photo("habitacion-01", "Habitación", "Una habitación de ambiente cálido pensada para el descanso.", "Habitación con cama y techo de madera", "habitacion", pdfImage("habitacion-01")),
      ...
      photo("bano-tina", "Baño con tina", "Una vista del baño y la tina visible en la fotografía.", "Baño con tina de hidromasaje visible", "bano", pdfImage("bano-tina")),
